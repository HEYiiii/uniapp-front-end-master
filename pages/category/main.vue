<template>
	<view>
		<view class='cu-custom'>
			<view class="cu-bar fixed bg-shadeTop" :style="['height:'+this.CustomBar+'px','padding-top:'+this.StatusBar+'px']">
		...
		<view class="VerticalBox">
			<scroll-view class="VerticalNav nav" scroll-y scroll-with-animation :scroll-top="VerticalNavTop" style="height:calc(100vh - 100rpx)">
				<view :class="['cu-item', index==TabCur?'text-blue cur':'']" v-for="(item,index) in list" :key="index" @tap='tabSelect'
				 :data-id="index">
					{{item.name}}
				</view>
			</scroll-view>
			 ...
			 <!--  -->
				<view class="padding-top padding-lr" v-for="(item,index) in list" :key="index" :id="'main-'+index">
					...
					<view class="cu-list menu-avatar">
						<view class="cu-item" @tap="toDetail(detailItem.id,detailItem.mininame,detailItem.description)" v-for="(detailItem,detailIndex) in categoryList "
						 v-if="detailItem.name==item.name" :key="detailIndex" :id="detailItem.id">
	...
	export default {
		...
		methods: {
			...
			toDetail(categoryId, name, description) {
				//跳转到详情页+传递参数index
				wx.navigateTo({
					url: '/pages/category_product/main?categoryId=' + categoryId + '&categoryName=' + name + '&categoryDescription=' +
						description,
					success: (result) => {},
					fail: () => {},
					complete: () => {}
